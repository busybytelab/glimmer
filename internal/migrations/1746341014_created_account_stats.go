...
func init() {
	m.Register(func(app core.App) error {
		jsonData := `{
			...
			"createRule": null,
			"deleteRule": null,
			"listRule": "@request.auth.id ?= @collection.accounts.owner && @collection.accounts.id ?= id",
			"updateRule": null,
			"viewRule": "@request.auth.id ?= @collection.accounts.owner && @collection.accounts.id ?= id",
